...
import { ShieldAlert, Calendar, CheckSquare, BarChart3, Clock, BookOpen, CalendarDays, User, LogOut, Key, X, ChevronDown } from 'lucide-react';
...
export default function OrtuDashboardPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Topbar / Navbar Khusus Orang Tua */}
      <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="font-bold text-lg text-emerald-700 flex items-center gap-2">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 hover:bg-slate-50 p-2 rounded-md transition-colors border border-slate-200"
          >
            <div className="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 font-bold">
              <User className="w-4 h-4" />
            </div>
            <span className="hidden sm:block text-sm font-medium text-slate-700">Profil Saya</span>
            <ChevronDown className="w-4 h-4 text-slate-500" />
          </button>

          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white text-slate-800 rounded-lg shadow-lg border border-slate-200 z-50 overflow-hidden">
              <div className="py-1">
                <button
                  onClick={() => {
      ...
      <div id="jadwal-pelajaran" className="grid grid-cols-1 lg:grid-cols-2 gap-6 scroll-mt-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 max-h-75 pr-2 custom-scrollbar">
              ...
              <div className="space-y-3">
                {schedules.filter(s => s.day === selectedDay)
                  ...
                  <div key={sch._id} className="flex items-center p-3 rounded-lg border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors">
                    <div className="shrink-0 w-12 h-12 bg-indigo-100 text-indigo-600 rounded-lg flex flex-col items-center justify-center font-bold text-sm">
